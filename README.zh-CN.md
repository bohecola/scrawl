...
| Monaco 编辑器 | VS Code 同款编辑器，带语法高亮、自动补全、跨文件 import 路径建议、TypeScript 类型检查和 Emmet 缩写展开（`!`、`div.card>li*5`、`m10`…）；多标签页，字号、编辑器主题、文件图标都可在设置里调。 |
...
| 内置 Demo | 资源面板（右上角）内置一组可直接运行的示例片段，可一键存到本地再改。 |
